    ...
    <div onClick={close} style={{ position: "fixed", inset: 0, background: "rgba(10,15,30,0.6)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ position: "relative", background: "#fff", maxWidth: 420, width: "100%", borderRadius: 0, overflow: "hidden", boxShadow: "0 20px 60px rgba(0,0,0,0.35)" }}>
        ...
        <div style={{
          ...
          <div style={{ font: '800 13px/1 "Inter", sans-serif', letterSpacing: "2px", color: "#cfe3f7", marginBottom: 14, textTransform: "uppercase" }}>
            Celebrate 250 Years of America
          </div>
          <div style={{ font: '900 56px/1 "Inter", sans-serif', color: "#fff", margin: "6px 0" }}>
            10% <span style={{ color: "#ffd166" }}>OFF</span>
          </div>
      ...
      <section style={{ position: "relative", background: "#fff", overflow: "hidden" }}>
        ...
        <div style={{
          ...
          <div style={{ minWidth: 0 }}>
            ...
            {photos.length > 1 && (
              <div style={{ display: "flex", justifyContent: "center", gap: 8, marginTop: 10, overflowX: "auto", WebkitOverflowScrolling: "touch", paddingBottom: 4 }}>
                {photos.map((p, i) => (
                  <button key={i} onClick={() => setActiveIdx(i)} style={{
    ...
    <footer style={{ background: "#f4f6f9", padding: isMobile ? "48px 24px" : "64px 80px" }}>
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1.6fr 1fr 1fr 1fr 1fr", gap: isMobile ? 40 : 40, alignItems: "start" }}>
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <a href="/privacy-policy" target="_blank" rel="noopener" style={btn}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="square" aria-hidden="true">
                <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/>
              </svg>
              Privacy Policy
