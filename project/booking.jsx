...
function StepPayment({ state, setState, onNext, onBack }) {
  ...
  async function handlePay() {
    ...
    // ── Where real charging would happen ────────────────────────────────
    // In production, send paymentMethod.id and the booking details to your
    // backend, which calls Stripe with your SECRET key:
    //
    ...
    //     await stripe.confirmCardPayment(result.client_secret); // 3D Secure
    //   }
    //   if (!result.success) { setCardError(result.message); setProcessing(false); return; }
    //
    // For this prototype, we skip the backend and persist the booking
    // locally so the conflict-blocking demo still works.
    Bookings.add(state.trailerId, {
      id: "TVR-" + Date.now().toString(36).toUpperCase(),
      pickup: state.pickup,
