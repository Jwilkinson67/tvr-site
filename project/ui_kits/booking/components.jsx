...
/* ----------- Primitives ----------- */
function Button({ variant = "primary", children, onClick, disabled, full, ...rest }) {
  const base = {
    ...
    <aside style={{
      ...
      {trailer ? (
        <div style={{ background: "#f4f6f9", padding: 16, marginBottom: 16, display: "flex", alignItems: "center", gap: 14, overflow: "hidden" }}>
          <img src={trailer.photo} alt="" style={{ width: 80, height: "auto", transform: trailer.id === "hauler" ? "scale(1.18)" : "none", transformOrigin: "center center" }}/>
          <div>
...
/* ----------- Trailer data ----------- */
const TRAILERS = {
  cargo:  { id: "cargo",  name: "Enclosed Cargo 8.5 × 16", kicker: "Enclosed cargo · 7,000 lb GVWR",
            photo: "../../assets/fleet/cargo-8x16.png", daily: 125 },
  hauler: { id: "hauler", name: "Car Hauler / Equipment 8.5 × 20", kicker: "Open car hauler · 9,990 lb GVWR",
            photo: "../../assets/fleet/hauler-8x20.png", daily: 110 },
};
    ...
    <StepShell title="Pick a trailer and dates" kicker="STEP 1 · TRAILER & DATES">
        ...
        {Object.values(TRAILERS).map(t => {
          ...
          return (
            <button key={t.id} onClick={() => setState({...state, trailerId: t.id})} style={{
              ...
              <div style={{ background: "#f4f6f9", padding: 16, display: "flex", justifyContent: "center", overflow: "hidden" }}>
                <img src={t.photo} alt="" style={{ width: "85%", maxHeight: 130, transform: t.id === "hauler" ? "scale(1.18)" : "none", transformOrigin: "center center" }}/>
              </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 24 }}>
        <Field label="Pickup date">
          <Input type="date" value={state.pickup} onChange={v => {
            const d = computeDays(v, state.dropoff);
            setState({...state, pickup: v, days: d});
          }}/>
        </Field>
        <Field label="Return date">
          <Input type="date" value={state.dropoff} onChange={v => {
...
function StepDocs({ state, setState, onNext, onBack }) {
  const docs = [
    { id: "license", t: "Driver's license", s: "Front and back. JPG or PDF, max 10 MB." },
  ...
  function fakeUpload(id) {
    setState({...state, docs: {...(state.docs || {}), [id]: { name: id === "license" ? "license_front.jpg" : "geico_policy.pdf", size: id === "license" ? "2.4 MB" : "183 KB", status: "uploaded" }}});
  }
  return (
          ...
          return (
            <div key={d.id} style={{
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ font: '300 13px/1.5 "Inter", sans-serif', color: "#6b6b6b" }}>
                  {f ? <><span style={{ color: "#262626", fontWeight: 700 }}>{f.name}</span> · {f.size} · <span style={{ color: "#22c55e", fontWeight: 700 }}>uploaded</span></> : d.s}
                </div>
              </div>
              {f ? (
                <Button variant="ghost" onClick={() => fakeUpload(d.id)}>Replace</Button>
              ) : (
                <Button variant="secondary" onClick={() => fakeUpload(d.id)}>Upload</Button>
              )}
            </div>
          );
        })}
...
function StepPayment({ state, setState, onNext, onBack }) {
  const valid = state.card && state.exp && state.cvv && state.zip;
  return (
    <StepShell title="Payment" kicker="STEP 6 · PAYMENT">
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 32 }}>
        <div style={{ gridColumn: "1 / -1" }}>
          <Field label="Card number">
            <Input value={state.card} onChange={v => setState({...state, card: v})} placeholder="1234  5678  9012  3456"/>
          </Field>
        </div>
        <Field label="Expiry">
          <Input value={state.exp} onChange={v => setState({...state, exp: v})} placeholder="MM / YY"/>
        </Field>
        <Field label="CVV">
          <Input value={state.cvv} onChange={v => setState({...state, cvv: v})} placeholder="123"/>
        </Field>
        <Field label="Cardholder name">
          <Input value={state.cardName} onChange={v => setState({...state, cardName: v})} placeholder={state.name || ""}/>
        </Field>
        <Field label="ZIP code">
          <Input value={state.zip} onChange={v => setState({...state, zip: v})} placeholder="37406"/>
        </Field>
      </div>

      <div style={{ padding: 16, background: "#f4f6f9", display: "flex", gap: 12, alignItems: "center", marginBottom: 32 }}>
        ...
        <span style={{ font: '300 13px/1.55 "Inter", sans-serif', color: "#3c3c3c" }}>
          Encrypted in transit. TVR doesn't store full card numbers.
        </span>
      </div>

      <StepFooter onBack={onBack} onNext={onNext} nextLabel="Pay deposit & confirm" disabled={!valid}/>
    </StepShell>
  );
}
  ...
  return (
    <StepShell title={null} kicker={null} wide>
      ...
      <div style={{ display: "flex", gap: 12 }}>
        ...
        <Button variant="secondary" onClick={onReset}>New reservation</Button>
      </div>
    </StepShell>
  );
}
